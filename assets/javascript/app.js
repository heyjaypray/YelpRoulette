...
$("#search-btn").on("click", function() {
	...
	if (formIsValid === true) {
		...
		navigator.geolocation.getCurrentPosition(function(position) {
		    ...
		    var yurl  		= "https://api.yelp.com/v3/businesses/search?term=" + keyword + "&location=" 
		     					+ position.coords.latitude + "," + position.coords.longitude + "&limit=50";
		    console.log(yurl);
			...
			$.ajax({
				...
				if (response.businesses.length > 0) {
			        ...
			        setTimeout(function() { 
				        ...
				        var gurl = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial" + 
				        			"&origins=" + origin_lat + "," + origin_long + "&destinations=" + lati + 
				        			"," + long + "&key=" + "AIzaSyBOrrP3F9BuMmSkdtiCkVdvjNFtbYQCZAE";
